...
//-----------------------------------------------------------------------------
extern void GetFogColor( fogparams_t *pFogParams, float *pColor );
extern float GetFogMaxDensity( fogparams_t *pFogParams );
extern bool GetFogEnable( fogparams_t *pFogParams );
extern float GetFogStart( fogparams_t *pFogParams );
extern float GetFogEnd( fogparams_t *pFogParams );
extern float GetSkyboxFogStart();
extern float GetSkyboxFogEnd();
extern float GetSkyboxFogMaxDensity();
extern void GetSkyboxFogColor( float *pColor );
// set any of these to use the maps fog
extern ConVar fog_enableskybox;

static void PositionHudPanels( CUtlVector< vgui::VPANEL > &list, const CViewSetup &view )
{
	FOR_EACH_VEC( list, i )
	{
		if ( list[i] )
			vgui::ipanel()->SetSize( list[i], view.width, view.height );
	}
}
