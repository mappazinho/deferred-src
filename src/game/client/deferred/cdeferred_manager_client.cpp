...
{
	AssertMsg( view == NULL, "viewrender already allocated?!" );
	...
	{
		if ( !bSM30 && !bForceDeferred )
			Warning( "Deferred lighting requires DirectX support level 95 (shader model 3.0); using the stock renderer.\n" );
		else
			Warning( "Deferred shader extension was unavailable; using the stock renderer.\n" );
	}
	else
	{
