	...
	SHADER_DRAW
	{
		...
		SHADOW_STATE
		{
			...
			// invokes the shared shader build with -force30.
			{
				...
				SET_STATIC_VERTEX_SHADER_COMBO( VERTEXCOLOR,  bHasVertexAlpha );
				SET_STATIC_VERTEX_SHADER_COMBO( LIGHTING_PREVIEW, nLightingPreviewMode != 0 );
				SET_STATIC_VERTEX_SHADER_COMBO_OVERRIDE_DEFAULT( DOPIXELFOG, 0 );
				SET_STATIC_VERTEX_SHADER_COMBO_OVERRIDE_DEFAULT( HARDWAREFOGBLEND, 0 );
...
//				SET_DYNAMIC_VERTEX_SHADER_COMBO( TESSELLATION, 0 );             // JasonM TODO: set this appropriately when we care about decals on subds				
...
#else
			{
				...
				SET_DYNAMIC_VERTEX_SHADER_COMBO( SKINNING, bFastVertexTextures && pShaderAPI->GetCurrentNumBones() > 0 );
				SET_DYNAMIC_VERTEX_SHADER_COMBO( COMPRESSED_VERTS, (int)vertexCompression );
				SET_DYNAMIC_VERTEX_SHADER_COMBO_OVERRIDE_DEFAULT( DOWATERFOG, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
				SET_DYNAMIC_VERTEX_SHADER_COMBO_OVERRIDE_DEFAULT( MORPHING, bHWMorphing );
//				SET_DYNAMIC_VERTEX_SHADER_COMBO( TESSELLATION, 0 );             // JasonM TODO: set this appropriately when we care about decals on subds				
				SET_DYNAMIC_VERTEX_SHADER( decalmodulate_vs30 );

				DECLARE_DYNAMIC_PIXEL_SHADER( decalmodulate_ps30 );
				SET_DYNAMIC_PIXEL_SHADER_COMBO_OVERRIDE_DEFAULT( PIXELFOGTYPE, pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z );
